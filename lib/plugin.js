...
class Plugin {
  constructor(name) {
    // strip the haraka-plugin- prefix for identity, as core Haraka does, so
    // results land under the short name; path resolution keeps the full name
    this.name = name.startsWith('haraka-plugin-') ? name.slice(14) : name
    this.base = {}
    this.timeout = 10
    ...
    logger.add_log_methods(this, this.name)
  ...
  inherits(parent_name) {
    ...
    // A core base plugin (e.g. 'auth/auth_base', 'queue/discard') is
    // not resolvable from a standalone plugin's test environment;
    // _get_plugin_path() then falls through to the current plugin's
    // own package.json, so `parent_plugin` is really *this* plugin.
    // Inheriting from ourselves and re-invoking register() recurses
    // until the stack overflows. Treat an unresolved core base plugin
    // as a no-op instead.
    if (parent_plugin.plugin_path === this.plugin_path) {
      this.logdebug(`inherits('${parent_name}') not resolvable in test fixtures; skipping`)
      return
    }
