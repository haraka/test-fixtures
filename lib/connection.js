...
class Connection {
  ...
  respond(code, msg, func) {
    // Haraka's real respond() does not require a callback; several
    // responders (e.g. the mail_respond accept path) call it with none.
  ...
  get_capabilities() {
    const capabilities = ['PIPELINING', '8BITMIME', `SIZE ${this.cfg?.main?.max_bytes || 0}`]
...
exports.createConnection = function (client = {}, server = {}, cfg = {}) {
  if (!cfg || Object.keys(cfg).length === 0) {
    cfg = config.get('smtp.ini', {
      booleans: ['+main.smtputf8', '+headers.add_received', '+headers.clean_auth_results'],
    })
  }
