...
function Transaction () {
    ...
    this.header = {
        headers: {}, headers_decoded: {}, header_list: [], options: {},
        parse: function () { true; },
        add: function (key, val) {this.headers[key] = this.headers[key] || []; this.headers[key].push(val);},
        add_end: function (key, val) { this.add(key, val); },
        get: function (key) { return (this.headers[key] || []).join('\n');},
        get_all: function (key) { return this.headers[key] || [];},
        remove: function (key) { delete this.headers[key]; },
    };
    this.message_stream = {
        add_line: function () { return true; },
        add_line_end: function () { return true; },
    };
