 ...
 *     passing the expected 4.0 hook arity so the dispatcher can tell a
 *     genuine 4.0 async hook from a 3.x callback hook declared `async`
 *
 * For plugins that export the hook method under a non-default name
 * (e.g. aliases registers 'aliases' for 'rcpt' via
 * `this.register_hook('rcpt', 'aliases')`), the helper finds it via
 * plugin.hooks (populated by register_hook). Callers can also pass
...
const noArg = (hookName) => (plugin, conn, method) =>
  invoke(plugin, resolveMethod(plugin, hookName, method), [conn], { asyncArity: 1 })

const stringArg = (hookName) => (plugin, conn, arg, method) =>
  invoke(plugin, resolveMethod(plugin, hookName, method), [conn, arg], { asyncArity: 2 })

const addressArg = (hookName) => (plugin, conn, addr, params, method) => {
  // A bare string in the `params` slot means the caller is using the
  // 4-arg form to pass a method override (params is always an object).
  if (typeof params === 'string' && method === undefined) {
    method = params
    params = undefined
  }
  return invoke(plugin, resolveMethod(plugin, hookName, method), [conn, [toAddress(addr), params ?? {}]], {
    asyncArity: 2,
  })
}
...
exports.callRcptOk = (plugin, conn, rcpt, method) =>
  invoke(plugin, resolveMethod(plugin, 'rcpt_ok', method), [conn, toAddress(rcpt)], { asyncArity: 2 })
...
exports.callQueueOk = (plugin, conn, msg, method) =>
  invoke(plugin, resolveMethod(plugin, 'queue_ok', method), [conn, msg], { asyncArity: 2 })
