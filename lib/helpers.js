 ...
 * @param {object}  [opts.server]                - assigned to connection.server; use for
 *                                                 server.notes.* injection (e.g. shared redis handle)
 * @param {string}  [opts.host]                  - remote.host (rDNS hostname)
 * @param {boolean} [opts.silent=false]          - replace every log method with a no-op
 * @param {boolean} [opts.stubLogs=false]        - replace every log method with a recording stub()
 * @returns {Connection}
 */
exports.makeConnection = ({
  ...
  conn.remote.ip = ip
  if (host) conn.remote.host = host
  if (helo) {
    conn.hello.host = helo
    conn.hello.verb = 'EHLO'
  }
  if (silent || stubLogs) {
    const replace = silent ? () => () => {} : () => stub()
    for (const m of LOG_METHODS) conn[m] = replace()
  }
  ...
  // Set relaying through the setter so it lands on transaction._relaying when
  // a transaction exists (matches Connection.relaying semantics).
  if (relaying) conn.relaying = true
